...
| Vlog の合成 | 全ページを `xstack` で合成 | 1コマのときは `xstack` を使わずコマをそのまま使う | **xstack は入力2本以上が必須**。個人ログ (F-202) は必ず1人なので、分けないと全ユーザーの個人Vlogが失敗する |
| 氏名の焼き込み | `drawtext=text='山田 12\:04'` | `textfile=` + `expansion=none` | `text=` は `\ ' : % , [ ] ;` の多重エスケープが必要で、日本語氏名で事故る。ファイル渡しにすればエスケープ自体が不要になる |
| Vlog に入れるスロット | 全スロット | 誰か1人でも撮っているスロットのみ | 全員プレースホルダの2秒が並ぶだけの区間を作らない。1本も撮影が無い日は Vlog 自体を作らない |
| FFmpeg の入手 | （記載なし） | Docker イメージに apt の `ffmpeg` と `fonts-noto-cjk` | `ffmpeg-static` には **drawtext が入っていない**。CJKフォントが無いと氏名が豆腐になる。Dockerfile のビルド時に両方を検証している |
